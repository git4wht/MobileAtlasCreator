...
public abstract class AtlasCreator {
	...
	public static final Charset TEXT_FILE_CHARSET = StandardCharsets.ISO_8859_1;
	...
	public void initializeMap(MapInterface map, TileProvider mapTileProvider) {
		...
		if (this.mapSource instanceof InitializableMapSource) {
			try {
				((InitializableMapSource) this.mapSource).initialize();
			} catch (MapSourceInitializationException e) {
				throw new RuntimeException("Initialization of MapSource \"" + //
						this.mapSource.getName() + "\" failed: " + e.getMessage(), e);
			}
		}
