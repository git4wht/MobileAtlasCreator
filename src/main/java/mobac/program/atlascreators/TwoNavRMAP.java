...
public class TwoNavRMAP extends AtlasCreator {
	...
	private class ZoomLevel {
		...
		private byte[] getTileData(TileImageDataWriter writer, ZoomLevel source, int x, int y) throws IOException {
			...
			BufferedImage bi22 = (x + 1 < source.xTiles) && (y + 1 < source.yTiles)
					? loadJpegAtOffset(source.jpegOffsets[x + 1][y + 1])
					: null;
		...
		private void shrinkFrom(ZoomLevel source) {
			try (TileImageDataWriter writer = new TileImageJpegDataWriter(0.9)) {
				writeHeader();
				...
				for (int m = 0; m < layer.getMapCount(); m++) {
					if ((mapN.getZoom() == layer.getMap(m).getZoom()) && (m != n)) {
						throw new AtlasTestException(
								"Several maps with the same zoom level within the same layer " + "are not supported!");
					}
				}
			}
		}
	...
	public void initLayerCreation(LayerInterface layer) throws IOException {
		...
		for (int n = 0; n < rmapFile.zoomLevels.length; n++) {
			...
			rmapFile.zoomLevels[n].xTiles = (int) Math
					.ceil((double) rmapFile.zoomLevels[n].width / (double) rmapFile.tileWidth);
			rmapFile.zoomLevels[n].yTiles = (int) Math
					.ceil((double) rmapFile.zoomLevels[n].height / (double) rmapFile.tileHeight);
		...
		for (int n = 0; n < rmapFile.zoomLevels.length; n++) {
			log.trace(String.format("zoomLevels[%d] zoom=%d %dx%d pixels, %dx%d tiles %s", rmapFile.zoomLevels[n].index,
					rmapFile.zoomLevels[n].zoom, rmapFile.zoomLevels[n].width, rmapFile.zoomLevels[n].height,
					rmapFile.zoomLevels[n].xTiles, rmapFile.zoomLevels[n].yTiles,
