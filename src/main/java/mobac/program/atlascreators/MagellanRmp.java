...
public class MagellanRmp extends AtlasCreator {
	...
	TileImageDataWriterBuilder tileWriterBuilder = null;
		...
		for (LayerInterface layer : atlas) {
			for (MapInterface map : layer) {
				...
				if (map.getParameters() != null) {
					if (!(map.getParameters().getFormat().getDataWriterBuilder() instanceof TileImageJpegDataWriterBuilder))
	 ...
	 * @throws IOException
	 */
	public RmpLayer createLayer(MultiImage si, int layer)
			throws InterruptedException, MapCreationException, IOException {
		...
		try (TileImageDataWriter tileWriter = tileWriterBuilder.build()) {
			/*
			...
			for (int x = x_start; x < x_end; x++) {
				for (int y = y_start; y < y_end; y++) {
					...
					/* --- Create tile --- */
					BoundingRect subrect = new BoundingRect(y * tile_height - 90, (y + 1) * tile_height - 90,
							x * tile_width - 180, (x + 1) * tile_width - 180);
		...
		/* --- Build the TLM file --- */
		rmpLayer.buildTLMFile(tile_width, tile_height, rect.getWest(), rect.getEast(), rect.getNorth(),
				rect.getSouth());
